...
//    cls();
    ...
    for (i = 0; i < 29 * 3 ; i++)
    {
        ...
        if(i+n<dirnum){
            ...
            printchar(']');
            for(j=11-strlen(filenames[i+n]);j>0;j--) printchar(' ');
        }
        else if(i+n<filenum){
            setcursorcolor(7);
            printstr(filenames[i + n]);
            for(j=13-strlen(filenames[i+n]);j>0;j--) printchar(' ');
        }
        else for(j=0;j<13;j++) printchar(' ');
    }
}
...
unsigned char *fileselect(void)
{
    ...
    while (1)
    {
        ...
        fr = f_findfirst(&dj, &fno, path, "*.*");
        if (fr)
            disperror("Findfirst Error.", fr);
        ...
        do{
            ...
            if(keycountLEFT>30) key|=keycountLEFT;
